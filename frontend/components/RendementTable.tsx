...
export function RendementTable({
  ...
  // Default: richest net yield first (rows already arrive yield-desc).
  ...
  const items = useMemo<(RdRow | typeof SEP)[]>(() => {
    const byYield = (a: RdRow, b: RdRow) => b.yieldNet - a.yieldNet;
    if (!userSorted) {
      const keep = rows.filter((r) => verdictTone(mode, r.verdict) !== "low").sort(byYield);
      const ceder = rows.filter((r) => verdictTone(mode, r.verdict) === "low").sort(byYield);
