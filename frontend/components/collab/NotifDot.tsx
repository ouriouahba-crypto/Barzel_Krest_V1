...
export function NotifDot({ count, showCount = false }: { count: number; showCount?: boolean }) {
  ...
  if (showCount) {
    return (
      <span
        role="status"
        aria-label={`${count} nouveau${count > 1 ? "x" : ""} message${count > 1 ? "s" : ""}`}
  ...
  return (
    <span role="status" aria-label="Nouveau contenu dans la discussion" className="relative inline-flex h-2 w-2">
